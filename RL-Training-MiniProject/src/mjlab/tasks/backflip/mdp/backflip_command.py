...
# ... (UniformVelocityCommand removed for brevity, assuming you only need the Backflip part) ...
...
class BackflipCommand(CommandTerm):
    ...
    def _update_command(self) -> None:
        ...
        # We start by squatting down to 0.15m to load the "spring"
        mask_squat = (t < 0.25)
        ...
        # Spin Phase (0.3 to 0.7)
        # We start spinning slightly after takeoff and finish BEFORE landing.
        mask_spin = (t >= 0.3) & (t < 0.7)
        t_spin = (t[mask_spin] - 0.3) / 0.4
        target_p[mask_spin] = -2 * np.pi * t_spin
        
        # Post-Spin / Landing Phase (0.7 to 1.0)
        # Enforce flat orientation (2pi is effectively 0) so it lands on feet
    ...
    def _debug_vis_impl(self, visualizer: "DebugVisualizer") -> None:
        ...
        # Visualizing Target Height
        cmd = self.bf_command_b[batch].cpu().numpy()
        target_pos = base_pos.copy()
        target_pos[2] = cmd[0]
        # (Assuming visualizer API exists, logic here is placeholder based on your file)


@dataclass(kw_only=True)
class BackflipCommandCfg(CommandTermCfg):
    asset_name: str
    cycle_time: float = 2.0 # Increased for stability
